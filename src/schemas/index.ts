...
export const AnalysisSchema = z.object({
  ...
  summaryBullets: z.array(z.string()).min(5).max(8),
  keyClaims: z.array(KeyClaimSchema).min(3).max(7),
  questions: z.array(QuestionSchema).min(5).max(12),
  missingAblations: z.array(MissingAblationSchema).min(3).max(10),
  potentialIssues: z.array(PotentialIssueSchema).min(3).max(10),
  replicationChecklist: z.array(z.string()).min(5).max(12),
  nextWeekTests: z.array(z.string()).min(2).max(5),
  modelMeta: z.object({
    provider: z.string(),
    model: z.string(),
    timestamp: z.number(),
